/*------------------------------------------------------------------------
  ...
  Fill, Clear and SetPWMBits functions of the libadargbmatrix library.
...
using adargb_matrix::GPIO;
using adargb_matrix::AdaRGBMatrix;
...
static PyObject *AdaRGBmatrix_new(
  PyTypeObject *type, PyObject *arg, PyObject *kw) {
	AdaRGBmatrixObject *self = NULL;
	int              rows, chain;

	if((PyTuple_Size(arg) == 2) &&
	   PyArg_ParseTuple(arg, "II", &rows, &chain)) {
		if((self = (AdaRGBmatrixObject *)type->tp_alloc(type, 0))) {
			self->matrix = new AdaRGBMatrix(&io, rows, chain);
			Py_INCREF(self);
		}
	}
...
static int AdaRGBmatrix_init(AdaRGBmatrixObject *self, PyObject *arg) {
	self->matrix->Clear();
	return 0;
}

static void AdaRGBmatrix_dealloc(AdaRGBmatrixObject *self) {
	delete self->matrix;
	self->ob_type->tp_free((PyObject *)self);
}

static PyObject *Clear(AdaRGBmatrixObject *self) {
	self->matrix->Clear();
...
static PyObject *Fill(AdaRGBmatrixObject *self, PyObject *arg) {
	uint32_t c;
	uint8_t  r, g, b;
	int      status=0;
...
static PyObject *SetPixel(AdaRGBmatrixObject *self, PyObject *arg) {
	uint32_t x, y, c;
	uint8_t  r, g, b;
	int      status=0;
...
static PyObject *SetBuffer(AdaRGBmatrixObject *self, PyObject *data) 
{
	Py_ssize_t count;
	int        w, h, offset, y, x;
	uint8_t    r, g, b;
...
static PyObject *SetImage(AdaRGBmatrixObject *self, PyObject *arg) {
...
static PyObject *SetPWMBits(AdaRGBmatrixObject *self, PyObject *arg) {
	uint8_t b;
...
static PyObject *SetWriteCycles(AdaRGBmatrixObject *self, PyObject *arg) {
	uint8_t b;
...
static PyTypeObject AdaRGBmatrixObjectType = {
	PyObject_HEAD_INIT(NULL)
	0,                              // ob_size (not used, always set to 0)
	"adargbmatrix.Adafruit_RGBmatrix", // tp_name (module name, object name)
	sizeof(AdaRGBmatrixObject),        // tp_basicsize
	0,                              // tp_itemsize
	(destructor)AdaRGBmatrix_dealloc,  // tp_dealloc
	0,                              // tp_print
	0,                              // tp_getattr
	0,                              // tp_setattr
	0,                              // tp_compare
	...
	0,                              // tp_descr_set
	0,                              // tp_dictoffset
	(initproc)AdaRGBmatrix_init,       // tp_init
	0,                              // tp_alloc
	AdaRGBmatrix_new,                  // tp_new
	0,                              // tp_free
};

PyMODINIT_FUNC initadargbmatrix(void) { // Module initialization function
	PyObject* m;

	if(io.Init() &&    // Set up GPIO pins.  MUST run as root.
	  (m = Py_InitModule("adargbmatrix", methods)) &&
	  (PyType_Ready(&AdaRGBmatrixObjectType) >= 0)) {
		Py_INCREF(&AdaRGBmatrixObjectType);
		PyModule_AddObject(m, "Adafruit_RGBmatrix",
		  (PyObject *)&AdaRGBmatrixObjectType);
	}
}
